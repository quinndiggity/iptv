...
int start_stream(const std::string &process_name,
  ...
  if (!common::file_system::is_directory_exist(feedback_dir)) {
    common::ErrnoError err =
        common::file_system::create_directory(feedback_dir, true);
    if (err) {
      ERROR_LOG() << "Failed to create FeeadBack directory, path: "
                  << feedback_dir;
    } else {
  ...
  iptv_cloud::stream::ProcessWrapper proc(process_name, feedback_dir,
                                          config_args, command_client, mem);
  int res = proc.Exec();
...
int stream_exec(const char *process_name, const struct cmd_args *args,
                void *config_args_ptr, void *command_client, void *mem) {
  if (!process_name || !args || !config_args_ptr || !command_client || !mem) {
    CRITICAL_LOG() << "Invalid arguments.";
  }

  const iptv_cloud::utils::ArgsMap config_args =
      *static_cast<iptv_cloud::utils::ArgsMap *>(config_args_ptr);
  const char *feedback_dir_ptr = args->feedback_dir;
  if (!feedback_dir_ptr) {
    CRITICAL_LOG() << "Define " FEEDBACK_DIR_FIELD
                      " variable and make it valid.";
  }

  common::logging::LOG_LEVEL logs_level =
      static_cast<common::logging::LOG_LEVEL>(args->log_level);
  common::libev::IoClient *client =
      static_cast<common::libev::IoClient *>(command_client);
  iptv_cloud::StreamStruct *smem = static_cast<iptv_cloud::StreamStruct *>(mem);
  return start_stream(process_name, feedback_dir_ptr, logs_level, config_args,
                      client, smem);
}
