...
void remove_notify_callback(gpointer user_data) { UNUSED(user_data); }
...
void RedirectGstLog(GstDebugCategory *category, GstDebugLevel level,
  ...
  case GST_LEVEL_ERROR:
    ERROR_LOG() << gst_debug_category_get_name(category) << " " << file << ":"
                << line << " " << function << " "
                << gst_debug_message_get(message);
    break;
  case GST_LEVEL_WARNING:
    WARNING_LOG() << gst_debug_category_get_name(category) << " " << file << ":"
                  << line << " " << function << " "
                  << gst_debug_message_get(message);
    break;
  case GST_LEVEL_FIXME:
    NOTICE_LOG() << gst_debug_category_get_name(category) << " " << file << ":"
                 << line << " " << function << " "
                 << gst_debug_message_get(message);
    break;
  case GST_LEVEL_INFO:
  case GST_LEVEL_LOG:
    INFO_LOG() << gst_debug_category_get_name(category) << " " << file << ":"
               << line << " " << function << " "
               << gst_debug_message_get(message);
    break;
  case GST_LEVEL_DEBUG:
    DEBUG_LOG() << gst_debug_category_get_name(category) << " " << file << ":"
                << line << " " << function << " "
                << gst_debug_message_get(message);
    break;
  default:
...
void streams_init(int argc, char **argv, EncoderType enc) {
  ...
  if (enc == GPU_MFX) {
    ...
    if (res == ERROR_RESULT_VALUE) {
      WARNING_LOG()
          << "Failed to set enviroment variable LIBVA_DRIVER_NAME to " MFX_ENV;
    }
  ...
  if (common::logging::CURRENT_LOG_LEVEL() ==
      common::logging::LOG_LEVEL_DEBUG) {
  ...
  DEBUG_LOG() << "Stream backend inited, LIBVA_DRIVER_NAME=" << va_dr_name
              << ", LIBVA_DRIVERS_PATH=" << va_dr_path;
}

void streams_deinit() { gst_deinit(); }

IBaseStream::IStreamClient::~IStreamClient() {}

IBaseStream::IBaseStream(Config *config, IStreamClient *client,
                         StreamStruct *stats)
    : common::IMetaClassInfo(), client_(client), api_(config), probe_in_(),
      probe_out_(), loop_(g_main_loop_new(ctx_holder::instance()->ctx, FALSE)),
      pipeline_(NULL), status_(SNEW), status_tick(0), no_data_panic_tick(0),
      stats_(stats), last_exit_status_(EXIT_INNER), is_live_(false),
      flags_(INITED_NOTHING), desire_flags_(INITED_NOTHING) {
...
void IBaseStream::RuntimeCleanUp() {
  const time_t max_life_time =
      common::time::current_mstime() / 1000 - 24 * 60 * 60;
  for (const OutputUri &output : api_->GetOutput()) {
    ...
    if (scheme == common::uri::Url::http) {
      const common::file_system::ascii_directory_string_path http_path =
          output.GetHttpRoot();
...
const char *IBaseStream::ClassName() const { return "IBaseStream"; }
...
ExitStatus IBaseStream::Exec() {
  ...
  guint main_timeout_id =
      g_timeout_add(main_timer_msecs, main_timer_callback, this);

  gst_bus_set_sync_handler(bus, sync_bus_callback, this,
                           remove_notify_callback);
...
bool IBaseStream::IsLive() const { return is_live_; }

bool IBaseStream::IsAudioInited() const { return flags_ & INITED_AUDIO; }

bool IBaseStream::IsVideoInited() const { return flags_ & INITED_VIDEO; }

void IBaseStream::RegisterAudioCaps(SupportedAudioCodecs saudio, GstCaps *caps,
                                    element_id_t id) {
  UNUSED(saudio);
  ...
  if (pad_struct) {
    ...
    if (gst_structure_get_int(pad_struct, "rate", &rate) &&
        gst_structure_get_int(pad_struct, "channels", &channels)) {
      common::media::DesireBytesPerSec kbps =
          common::media::CalculateDesireAudioBandwidthBytesPerSec(rate,
                                                                  channels);
      input_stream_info_t ins = stats_->input;
      common::media::DesireBytesPerSec prev =
          ins[id]->GetDesireBytesPerSecond();
...
void IBaseStream::RegisterVideoCaps(SupportedVideoCodecs svideo, GstCaps *caps,
                                    element_id_t id) {
  ...
  if (svideo == VIDEO_MPEG_CODEC) {
    ...
    if (pad_struct) {
      ...
      if (gst_structure_get_int(pad_struct, "width", &width) &&
        ...
        common::media::DesireBytesPerSec kbps =
            common::media::CalculateDesireMPEGBandwidthBytesPerSec(width,
                                                                   height);
        input_stream_info_t ins = stats_->input;
        common::media::DesireBytesPerSec prev =
            ins[id]->GetDesireBytesPerSecond();
    ...
    if (pad_struct) {
      ...
      if (gst_structure_get_int(pad_struct, "width", &width) &&
        ...
        if (gst_structure_get_fraction(pad_struct, "framerate",
                                       &value_numerator, &value_denominator)) {
          framerate = value_numerator / value_denominator;
        }

        common::media::DesireBytesPerSec kbps =
            common::media::CalculateDesireH264BandwidthBytesPerSec(
...
void IBaseStream::Stop() { SetPipelineState(GST_STATE_NULL); }

void IBaseStream::Pause() { SetPipelineState(GST_STATE_PAUSED); }
...
void IBaseStream::ResetDataWait() {
  no_data_panic_tick =
      GetElipsedTime() + no_data_panic_sec; // update no_data_panic timestamp
...
void IBaseStream::Quit(ExitStatus status) {
  ...
  GstStructure *result =
      gst_structure_new("exit_info", "status", G_TYPE_INT, status, NULL);
  GstMessage *message =
      gst_message_new_application(GST_OBJECT(pipeline), result);
...
StreamStruct *IBaseStream::GetStats() const { return stats_; }
...
StreamStatus IBaseStream::GetStatus() const { return status_; }

channel_id_t IBaseStream::GetID() const { return stats_->id; }

StreamType IBaseStream::GetType() const { return stats_->type; }

std::vector<Probe *> IBaseStream::GetProbesOut() const { return probe_out_; }

GstBusSyncReply IBaseStream::HandleSyncBusMessageReceived(GstBus *bus,
                                                          GstMessage *message) {
  UNUSED(bus);
  ...
  if (type == GST_MESSAGE_APPLICATION) {
    ...
    if (src == pipeline && g_main_loop_is_running(loop_)) {
      const GstStructure *exit_status_struct =
          gst_message_get_structure(message);
      const GValue *status_val =
          gst_structure_get_value(exit_status_struct, "status");
      gint exit_status = gvalue_cast<gint>(status_val);
      WARNING_LOG() << GetID()
                    << " Received exit command, status: " << exit_status;
...
void IBaseStream::OnOutputDataFailed() {
  WARNING_LOG() << GetID() << " There is no output data for a last "
                << no_data_panic_sec << " seconds.";
  Quit(EXIT_INNER);
}

void IBaseStream::OnOutputDataOK() { ResetDataWait(); }

void IBaseStream::OnInputDataFailed() {
  WARNING_LOG() << GetID() << " There is no input data for a last "
                << no_data_panic_sec << " seconds.";
  Quit(EXIT_INNER);
}
...
gboolean IBaseStream::HandleMainTimerTick() {
  ...
  if (up_time > no_data_panic_tick) { // check is stream in noraml state
    ...
    DEBUG_LOG() << GetID() << " NoData checkpoint: input eos (" << count_in_eos
                << "/" << input_stream_count << "), output eos ("
                << count_out_eos << "/" << output_stream_count
                << "), received bytes " << checkpoint_diff_in_total
    ...
    if (checkpoint_desire_in_total.IsValid() &&
      ...
      if (!checkpoint_desire_in_total.InRange(in_bytes_per_sec)) {
        NOTICE_LOG() << "Input bandwidth not in range of desire speed "
                     << checkpoint_desire_in_total.min
                     << " <= " << in_bytes_per_sec
                     << " <= " << checkpoint_desire_in_total.max;
      }
    }
  ...
  */
  ...
  if (IsActive()) {
    if (status_tick <= up_time) {
      status_tick =
          up_time + Config::report_delay_sec; // update status timestamp
      if (client_) {
...
gboolean IBaseStream::HandleAsyncBusMessageReceived(GstBus *bus,
                                                    GstMessage *message) {
  UNUSED(bus);
  ...
  case GST_MESSAGE_CLOCK_LOST: {
    ...
    INFO_LOG() << GetID()
               << " ASYNC GST_MESSAGE_CLOCK_LOST: " << GST_OBJECT_NAME(clock);
    break;
  }
  ...
  case GST_MESSAGE_EOS: {
    WARNING_LOG() << GetID()
                  << " Received end of stream: " << GST_OBJECT_NAME(src);
    if (client_) {
...
void IBaseStream::SetStatus(StreamStatus status) {
  ...
  INFO_LOG() << GetID() << " Changing status from "
             << common::ConvertToString(status_) << " to "
...
bool IBaseStream::IsActive() const { return status_ != SNULL; }
...
elements::Element *
IBaseStream::GetElementByName(const std::string &name) const {
  for (elements::Element *el : pipeline_elements_) {
    if (el->GetName() == name) {
      return el;
    }
...
GstPadProbeInfo *IBaseStream::CheckProbeData(Probe *probe,
...
GstPadProbeInfo *IBaseStream::CheckProbeDataOutput(Probe *probe,
                                                   GstPadProbeInfo *buff) {
  if (client_) {
...
GstStateChangeReturn IBaseStream::SetPipelineState(GstState state) {
  ...
  if (ret == GST_STATE_CHANGE_FAILURE) {
    ...
    WARNING_LOG() << "Unable to change pipeline state from:"
...
gboolean IBaseStream::main_timer_callback(gpointer user_data) {
  ...
  */
  ...
  DEBUG_LOG() << "HandleMainTimerTick time is: " << end_ts - start_ts
              << " msec.";
  return res;
}
...
Config *IBaseStream::GetApi() const { return api_; }

GstBusSyncReply IBaseStream::sync_bus_callback(GstBus *bus, GstMessage *message,
                                               gpointer user_data) {
  IBaseStream *stream = reinterpret_cast<IBaseStream *>(user_data);
  return stream->HandleSyncBusMessageReceived(bus, message);
}

gboolean IBaseStream::async_bus_callback(GstBus *bus, GstMessage *message,
                                         gpointer user_data) {
  UNUSED(bus);
...
bool IBaseStream::DumpIntoFile(
    const common::file_system::ascii_file_string_path &path) const {
  if (!path.IsValid()) {
    return false;
  }

  dumper::IDumper *dumper =
      dumper::DumpersFactory::GetInstance().CreateDumper(path);
  if (!dumper) {
    return false;
  }
