...
void ElementMP3Enc::SetTarget(guint target) { SetProperty("target", target); }
...
elements_line_t build_audio_converters(volume_t volume,
  ...
  elements::audio::ElementAudioConvert *audio_convert =
      new elements::audio::ElementAudioConvert(
          common::MemSPrintf(AUDIO_CONVERT_NAME_1U, audio_convert_id));
  ...
  if (achannels != DEFAULT_AUDIO_CHANNEL_COUNT) {
    elements::ElementCapsFilter *caps =
        new elements::ElementCapsFilter(common::MemSPrintf(
            AUDIO_CONVERT_CAPS_FILTER_NAME_1U, audio_convert_id));
    GstCaps *cap_convert = gst_caps_new_simple("audio/x-raw", "channels",
                                               G_TYPE_INT, achannels, NULL);
  ...
  if (volume != DEFAULT_VOLUME) {
    audio::ElementVolume *vol = new audio::ElementVolume(
        common::MemSPrintf(VOLUME_NAME_1U, audio_convert_id));
...
bool IsAACEncoder(const std::string &encoder) {
  return encoder == ElementFAAC::GetPluginName() ||
         encoder == ElementVoaacEnc::GetPluginName();
}
