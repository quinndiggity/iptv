...
#define CLIENT_START_STREAM_RESP_FAIL_1E                                       \
  GENEATATE_FAIL_FMT(CLIENT_START_STREAM, "'%s'")
// stop stream
#define CLIENT_STOP_STREAM_RESP_SUCCESS GENEATATE_SUCCESS(CLIENT_STOP_STREAM)
#define CLIENT_STOP_STREAM_RESP_FAIL_1E                                        \
  GENEATATE_FAIL_FMT(CLIENT_STOP_STREAM, "'%s'")
// stop restart
#define CLIENT_RESTART_STREAM_RESP_SUCCESS                                     \
  GENEATATE_SUCCESS(CLIENT_RESTART_STREAM)
#define CLIENT_RESTART_STREAM_RESP_FAIL_1E                                     \
  GENEATATE_FAIL_FMT(CLIENT_RESTART_STREAM, "'%s'")

// stop service
#define CLIENT_STOP_SERVICE_REQ_1E                                             \
  GENERATE_REQUEST_FMT_ARGS(CLIENT_STOP_SERVICE, "'%s'")
#define CLIENT_STOP_SERVICE_RESP_FAIL_1E                                       \
  GENEATATE_FAIL_FMT(CLIENT_STOP_SERVICE, "'%s'")
#define CLIENT_STOP_SERVICE_RESP_SUCCESS GENEATATE_SUCCESS(CLIENT_STOP_SERVICE)

// service state
#define CLIENT_STATE_SERVICE_RESP_SUCCESS_1E                                   \
  GENEATATE_SUCCESS_FMT(CLIENT_STATE_SERVICE, "'%s'")
...
protocol::responce_t StopServiceResponceSuccess(protocol::sequance_id_t id) {
  return common::protocols::three_way_handshake::MakeResponce(
      id, CLIENT_STOP_SERVICE_RESP_SUCCESS);
}

protocol::responce_t StopServiceResponceFail(protocol::sequance_id_t id,
                                             const std::string &error_text) {
  return common::protocols::three_way_handshake::MakeResponce(
      id, CLIENT_STOP_SERVICE_RESP_FAIL_1E, error_text);
}
