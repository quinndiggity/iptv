...
common::Error make_stream_info(const utils::ArgsMap& config_args, StreamInfo* sha) {
  if (!sha) {
  ...
  if (!utils::ArgsGetValue(config_args, ID_FIELD, &lsha.id)) {
    return common::make_error("Define " ID_FIELD " variable and make it valid.");
  }
...
void ProcessSlaveWrapper::TimerEmited(common::libev::IoLoop* server, common::libev::timer_id_t id) {
  if (ping_client_id_timer_ == id) {
    ...
    if (stats_) {
      ...
      bool res = stats_->SetKey(make_daemon_stats_id(node_id_), node_stats);  // node_id_ can be changed in runtime
      if (!res) {
...
common::Error ProcessSlaveWrapper::DaemonDataReceived(DaemonClient* dclient) {
...
common::Error ProcessSlaveWrapper::PipeDataReceived(pipe::PipeClient* pipe_client) {
  CHECK(loop_->IsLoopThread());
  std::string input_command;
...
void ProcessSlaveWrapper::DataReceived(common::libev::IoClient* client) {
  if (DaemonClient* dclient = dynamic_cast<DaemonClient*>(client)) {
    ...
    common::Error err = PipeDataReceived(pipe_client);
    if (err) {
      DEBUG_MSG_ERROR(err, common::logging::LOG_LEVEL_ERR);
      auto childs = loop_->GetChilds();
      for (size_t i = 0; i < childs.size(); ++i) {
...
common::Error ProcessSlaveWrapper::HandleRequestClientStopService(DaemonClient* dclient,
                                                                  protocol::sequance_id_t id,
                                                                  int argc,
                                                                  char* argv[]) {
  CHECK(loop_->IsLoopThread());
...
common::Error ProcessSlaveWrapper::CreateChildStream(common::libev::IoLoop* server, const StartStreamInfo& start_info) {
  CHECK(loop_->IsLoopThread());
  ...
  StreamInfo sha;
  common::Error err = make_stream_info(config_args, &sha);
  if (err) {
    return err;
  }
  ...
  if (stream) {
    ...
    return common::make_error(common::MemSPrintf("Stream with id: %s exist, skip request.", sha.id));
  }
  ...
  common::ErrnoError perr = create_pipe(&read_command_client, &write_requests_client);
  if (perr) {
    FreeSharedStreamStruct(&mem);
    return common::make_error_from_errno(perr);
  }

  int read_responce_client = 0;
  int write_responce_client = 0;
  common::ErrnoError perr2 = create_pipe(&read_responce_client, &write_responce_client);
  if (perr2) {
    FreeSharedStreamStruct(&mem);
    return common::make_error_from_errno(perr2);
  }
...
common::Error ProcessSlaveWrapper::HandleRequestChangedSourcesStream(pipe::PipeClient* pclient,
                                                                     protocol::sequance_id_t id,
                                                                     int argc,
                                                                     char* argv[]) {
  CHECK(loop_->IsLoopThread());
  if (argc > 1) {
    ...
    common::Error err = ch_sources_info.DeSerialize(jrequest_changed_sources);
    ...
    err = ch_sources_info.SerializeToString(&changed_str);
    if (err) {
      return common::Error();
    }
...
common::Error ProcessSlaveWrapper::HandleRequestStatisticStream(pipe::PipeClient* pclient,
                                                                protocol::sequance_id_t id,
                                                                int argc,
                                                                char* argv[]) {
  CHECK(loop_->IsLoopThread());
  if (argc > 1) {
    ...
    common::Error err = stat.DeSerialize(jrequest_stat);
    ...
    std::string status;
    err = stat.SerializeToString(&status);
    if (err) {
      return err;
    }
...
common::Error ProcessSlaveWrapper::HandleRequestClientStartStream(DaemonClient* dclient,
                                                                  protocol::sequance_id_t id,
                                                                  int argc,
                                                                  char* argv[]) {
  CHECK(loop_->IsLoopThread());
  ...
  if (argc > 1) {
    ...
    common::Error err = start_info.DeSerialize(jstart_info);
    ...
    err = CreateChildStream(server, start_info);
    if (err) {
...
common::Error ProcessSlaveWrapper::HandleRequestClientStopStream(DaemonClient* dclient,
                                                                 protocol::sequance_id_t id,
                                                                 int argc,
                                                                 char* argv[]) {
  CHECK(loop_->IsLoopThread());
  ...
  if (argc > 1) {
    ...
    common::Error err = stop_info.DeSerialize(jstop_info);
...
common::Error ProcessSlaveWrapper::HandleRequestClientRestartStream(DaemonClient* dclient,
                                                                    protocol::sequance_id_t id,
                                                                    int argc,
                                                                    char* argv[]) {
  CHECK(loop_->IsLoopThread());
  ...
  if (argc > 1) {
    ...
    common::Error err = restart_info.DeSerialize(jrestart_info);
...
common::Error ProcessSlaveWrapper::HandleRequestClientStateService(DaemonClient* dclient,
                                                                   protocol::sequance_id_t id,
                                                                   int argc,
                                                                   char* argv[]) {
  if (!dclient->IsVerified()) {
  ...
  if (argc > 1) {
    ...
    common::Error err = state_info.DeSerialize(jservice_state);
...
common::Error ProcessSlaveWrapper::HandleRequestClientActivate(DaemonClient* dclient,
                                                               protocol::sequance_id_t id,
                                                               int argc,
                                                               char* argv[]) {
  if (argc > 1) {
...
common::Error ProcessSlaveWrapper::HandleRequestServiceCommand(DaemonClient* dclient,
                                                               protocol::sequance_id_t id,
                                                               int argc,
                                                               char* argv[]) {
  UNUSED(id);
...
common::Error ProcessSlaveWrapper::HandleResponceServiceCommand(DaemonClient* dclient,
                                                                protocol::sequance_id_t id,
                                                                int argc,
                                                                char* argv[]) {
  UNUSED(dclient);
...
common::Error ProcessSlaveWrapper::HandleRequestStreamsCommand(pipe::PipeClient* pclient,
                                                               protocol::sequance_id_t id,
                                                               int argc,
                                                               char* argv[]) {
  UNUSED(id);
...
common::Error ProcessSlaveWrapper::HandleResponceStreamsCommand(pipe::PipeClient* pclient,
                                                                protocol::sequance_id_t id,
                                                                int argc,
                                                                char* argv[]) {
  UNUSED(pclient);
