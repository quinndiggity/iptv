...
json_object *MakeDirectoryStateResponce(const DirectoryState &dir) {
  ...
  json_object_object_add(obj, SAVE_DIRECTORY_FIELD_PATH,
                         json_object_new_string(path_str.c_str()));
  json_object_object_add(obj, SAVE_DIRECTORY_FIELD_STATUS,
                         json_object_new_boolean(dir.is_valid));
  json_object_object_add(obj, SAVE_DIRECTORY_FIELD_RESPONCE,
                         json_object_new_string(dir.error_str.c_str()));
...
struct Directories {
  explicit Directories(const iptv_cloud::server::StateServiceInfo &sinf)
      : job_dir(sinf.GetJobsDirectory(),
                STATE_SERVICE_INFO_JOBS_DIRECTORY_FIELD),
        timeshift_dir(sinf.GetTimeshiftsDirectory(),
                      STATE_SERVICE_INFO_TIMESHIFTS_DIRECTORY_FIELD),
        ...
        playlist_dir(sinf.GetPlaylistsDirectory(),
                     STATE_SERVICE_INFO_PLAYLIST_DIRECTORY_FIELD),
        dvb_dir(sinf.GetDvbDirectory(), STATE_SERVICE_INFO_DVB_DIRECTORY_FIELD),
        capture_card_dir(sinf.GetCaptureDirectory(),
                         STATE_SERVICE_INFO_CAPTURE_CARD_DIRECTORY_FIELD) {}
  ...
  bool IsValid() const {
    return job_dir.is_valid && timeshift_dir.is_valid && hls_dir.is_valid &&
           ads_dir.is_valid && playlist_dir.is_valid && dvb_dir.is_valid &&
           capture_card_dir.is_valid;
  }
};
...
iptv_cloud::utils::ArgsMap read_slave_config(const std::string &path) {
  ...
  while (getline(config, line)) {
    const std::pair<std::string, std::string> pair =
        iptv_cloud::utils::GetKeyValue(line, '=');
...
common::ErrnoError make_stream_info(const utils::ArgsMap &config_args,
                                    StreamInfo *sha) {
  if (!sha) {
    return common::make_errno_error_inval();
  }
  ...
  if (!utils::ArgsGetValue(config_args, ID_FIELD, &lsha.id)) {
    return common::make_errno_error(
        "Define " ID_FIELD " variable and make it valid.", EAGAIN);
  }

  if (!utils::ArgsGetValue(config_args, TYPE_FIELD, &lsha.type)) {
    return common::make_errno_error(
        "Define " TYPE_FIELD " variable and make it valid.", EAGAIN);
  }

  input_t input;
  if (!read_input(config_args, &input)) {
    return common::make_errno_error(
        "Define " INPUT_FIELD " variable and make it valid.", EAGAIN);
  }
  ...
  bool is_timeshift_and_rec =
      (lsha.type == TIMESHIFT_RECORDER && !is_multi_input) ||
      (lsha.type == CATCHUP && !is_multi_input);
  if (is_timeshift_and_rec) {
    ...
    if (!read_output(config_args, &output)) {
      return common::make_errno_error(
          "Define " OUTPUT_FIELD " variable and make it valid.", EAGAIN);
    }
    ...
    for (auto out_uri : output) {
      ...
      if (ouri.GetScheme() == common::uri::Url::http) {
        ...
        if (!common::file_system::is_directory_exist(http_root_str)) {
          common::ErrnoError errn =
              common::file_system::create_directory(http_root_str, true);
          if (errn) {
            return errn;
          }
        }
        common::ErrnoError errn =
            common::file_system::node_access(http_root_str);
        if (errn) {
          return errn;
        }
      }
...
ProcessSlaveWrapper::ProcessSlaveWrapper(const std::string &license_key)
    : stats_(nullptr), node_id_(),
      start_time_(common::time::current_mstime() / 1000), loop_(),
      license_key_(license_key), id_(), ping_client_id_timer_(INVALID_TIMER_ID),
      node_stats_timer_(INVALID_TIMER_ID), cleanup_timer_(INVALID_TIMER_ID),
...
int ProcessSlaveWrapper::SendStopDaemonRequest(const std::string &license) {
  ...
  common::ErrnoError err =
      common::net::connect(host, common::net::ST_SOCK_STREAM, 0, &client_info);
  if (err) {
    return EXIT_FAILURE;
  }
...
int ProcessSlaveWrapper::Exec(int argc, char **argv) {
  ...
  gpu_stats::IPerfMonitor *perf_monitor =
      gpu_stats::CreatePerfMonitor(&node_stats_->gpu_load);
  if (perf_monitor) {
    perf_thread = std::thread([perf_monitor] { perf_monitor->Exec(); });
  }

  int res = EXIT_FAILURE;
  common::libev::tcp::TcpServer *server =
      static_cast<common::libev::tcp::TcpServer *>(loop_);
...
void ProcessSlaveWrapper::PreLooped(common::libev::IoLoop *server) {
  ping_client_id_timer_ =
      server->CreateTimer(ping_timeout_clients_seconds, true);
...
void ProcessSlaveWrapper::Moved(common::libev::IoLoop *server,
...
void ProcessSlaveWrapper::TimerEmited(common::libev::IoLoop *server,
                                      common::libev::timer_id_t id) {
  if (ping_client_id_timer_ == id) {
    std::vector<common::libev::IoClient *> online_clients =
        server->GetClients();
    for (size_t i = 0; i < online_clients.size(); ++i) {
      ...
      if (dclient && dclient->IsVerified()) {
        ProtocoledDaemonClient *pdclient =
            static_cast<ProtocoledDaemonClient *>(dclient);
        ...
        if (err) {
          ...
          INFO_LOG() << "Pinged to client[" << client->GetFormatedName()
                     << "], from server[" << server->GetFormatedName() << "], "
    ...
    if (stats_) {
      ...
      uint64_t bytes_recv =
          (next_nshot.bytes_recv + node_stats_->prev_nshot.bytes_recv) / 2;
      uint64_t bytes_send =
          (next_nshot.bytes_send + node_stats_->prev_nshot.bytes_send) / 2;
      ...
      std::string uptime_str = common::MemSPrintf(
          "%lu %lu %lu", sshot.loads[0], sshot.loads[1], sshot.loads[2]);
      time_t cur_ts = common::time::current_mstime() / 1000;

      json_object *jstats = json_object_new_object();
      json_object_object_add(jstats, FIELD_STATS_CPU,
                             json_object_new_int(cpu_load * 100));
      json_object_object_add(jstats, FIELD_STATS_GPU,
                             json_object_new_int(node_stats_->gpu_load));
      json_object_object_add(jstats, FIELD_STATS_LOAD_AVERAGE,
                             json_object_new_string(uptime_str.c_str()));
      json_object_object_add(jstats, FIELD_STATS_MEMORY_TOTAL,
                             json_object_new_int64(mem_shot.total_ram));
      json_object_object_add(jstats, FIELD_STATS_MEMORY_FREE,
                             json_object_new_int64(mem_shot.free_ram));
      json_object_object_add(jstats, FIELD_STATS_MEMORY_AVAILIBLE,
                             json_object_new_int64(mem_shot.avail_ram));
      json_object_object_add(jstats, FIELD_STATS_HDD_TOTAL,
                             json_object_new_int64(hdd_shot.hdd_total));
      json_object_object_add(jstats, FIELD_STATS_HDD_FREE,
                             json_object_new_int64(hdd_shot.hdd_free));
      json_object_object_add(jstats, FIELD_STATS_BANDWIDTH_IN,
                             json_object_new_int64(bytes_recv));
      json_object_object_add(jstats, FIELD_STATS_BANDWIDTH_OUT,
                             json_object_new_int64(bytes_send));
      json_object_object_add(jstats, FIELD_STATS_UPTIME,
                             json_object_new_int64(sshot.uptime));
      json_object_object_add(jstats, FIELD_STATS_TIMESTAM,
                             json_object_new_int64(cur_ts));
      json_object_object_add(jstats, FIELD_STATS_PROJECT_VERSION,
                             json_object_new_string(PROJECT_VERSION_HUMAN));
      std::string node_stats = json_object_get_string(jstats);
      json_object_put(jstats);

      bool res =
          stats_->SetKey(make_daemon_stats_id(node_id_),
...
void ProcessSlaveWrapper::ChildStatusChanged(common::libev::IoChild *child,
                                             int status) {
...
common::ErrnoError
ProcessSlaveWrapper::DaemonDataReceived(DaemonClient *dclient) {
  CHECK(loop_->IsLoopThread());
  std::string input_command;
  ProtocoledDaemonClient *pclient =
      static_cast<ProtocoledDaemonClient *>(dclient);
...
common::ErrnoError
ProcessSlaveWrapper::PipeDataReceived(pipe::PipeClient *pipe_client) {
  CHECK(loop_->IsLoopThread());
  std::string input_command;
  pipe::ProtocoledPipeClient *pclient =
      static_cast<pipe::ProtocoledPipeClient *>(pipe_client);
  common::ErrnoError err = pclient->ReadCommand(&input_command);
  if (err) {
    return err; // i don't want handle spam, comand must be foramated according
                // protocol
...
void ProcessSlaveWrapper::DataReceived(common::libev::IoClient *client) {
  ...
  } else if (pipe::PipeClient *pipe_client =
                 dynamic_cast<pipe::PipeClient *>(client)) {
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientStopService(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  CHECK(loop_->IsLoopThread());
  if (argc > 1) {
    ...
    bool is_verified_request =
        stop_info.GetLicense() == license_key_ || dclient->IsVerified();
    ...
    if (cleanup_timer_ != INVALID_TIMER_ID) {
      ...
      protocol::responce_t resp =
          StopServiceResponceFail(id, "Stop service in progress...");
...
common::ErrnoError
ProcessSlaveWrapper::CreateChildStream(common::libev::IoLoop *server,
  ...
  if (stream) {
    ...
    return common::make_errno_error(
        common::MemSPrintf("Stream with id: %s exist, skip request.", sha.id),
        EINVAL);
  }
  ...
  pid_t pid = fork();
  if (pid == 0) { // child
    typedef int (*stream_exec_t)(
        const char *process_name, const struct cmd_args *cmd_args,
        void *config_args, void *command_client, void *mem);
    ...
    const std::string lib_full_path =
        common::file_system::make_path(absolute_source_dir, CORE_LIBRARY);
    ...
    char *error;
    stream_exec_t stream_exec_func =
        reinterpret_cast<stream_exec_t>(dlsym(handle, "stream_exec"));
    ...
    if (!utils::ArgsGetValue(cmd_args, FEEDBACK_DIR_FIELD, &feedback_dir)) {
      ERROR_LOG() << "Define " FEEDBACK_DIR_FIELD
    ...
    const std::string new_process_name =
        common::MemSPrintf(STREAMER_NAME " %s", sha.id);
    ...
    common::ErrnoError errn =
        common::file_system::close_descriptor(read_responce_client);
    if (errn) {
    ...
    pipe::PipeClient *client = new pipe::PipeClient(
        nullptr, read_command_client, write_responce_client);
    int res =
        stream_exec_func(new_name, &client_args, &config_args, client, mem);
    dlclose(handle);
    client->Close();
    delete client;
    _exit(res);
    ...
    // close not needed pipes
    common::ErrnoError errn =
        common::file_system::close_descriptor(read_command_client);
    if (errn) {
      DEBUG_MSG_ERROR(errn, common::logging::LOG_LEVEL_WARNING);
    }
    ...
    pipe::PipeClient *pipe_client = new pipe::PipeClient(
        server, read_responce_client, write_requests_client);
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestChangedSourcesStream(
    pipe::PipeClient *pclient, protocol::sequance_id_t id, int argc,
    char *argv[]) {
  CHECK(loop_->IsLoopThread());
  if (argc > 1) {
    ...
    common::Error err_des =
        ch_sources_info.DeSerialize(jrequest_changed_sources);
    ...
    pipe::ProtocoledPipeClient *pdclient =
        static_cast<pipe::ProtocoledPipeClient *>(pclient);
...
common::ErrnoError
ProcessSlaveWrapper::HandleRequestStatisticStream(pipe::PipeClient *pclient,
                                                  protocol::sequance_id_t id,
                                                  int argc, char *argv[]) {
  CHECK(loop_->IsLoopThread());
  if (argc > 1) {
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientStartStream(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  CHECK(loop_->IsLoopThread());
  ...
  if (argc > 1) {
    ...
    if (err) {
      protocol::responce_t resp =
          StartStreamResponceFail(id, err->GetDescription());
      pdclient->WriteResponce(resp);
      return err;
    }
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientStopStream(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  CHECK(loop_->IsLoopThread());
  if (!dclient->IsVerified()) {
    return common::make_errno_error_inval();
  ...
  if (argc > 1) {
    ...
    if (!chan) {
      protocol::responce_t resp =
          StopStreamResponceFail(id, "Stream not found.");
      pdclient->WriteResponce(resp);
      return common::ErrnoError();
    }
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientRestartStream(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  CHECK(loop_->IsLoopThread());
  if (!dclient->IsVerified()) {
    return common::make_errno_error_inval();
  ...
  if (argc > 1) {
    ...
    if (!chan) {
      protocol::responce_t resp =
          RestartStreamResponceFail(id, "Stream not found.");
      pdclient->WriteResponce(resp);
      return common::ErrnoError();
    }
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientStateService(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  if (!dclient->IsVerified()) {
    return common::make_errno_error_inval();
  }
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestClientActivate(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  if (argc > 1) {
...
common::ErrnoError ProcessSlaveWrapper::HandleRequestServiceCommand(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  UNUSED(id);
  UNUSED(argc);
  char *command = argv[0];
...
common::ErrnoError ProcessSlaveWrapper::HandleResponceServiceCommand(
    DaemonClient *dclient, protocol::sequance_id_t id, int argc, char *argv[]) {
  UNUSED(dclient);
  UNUSED(id);
  UNUSED(argc);
...
common::ErrnoError
ProcessSlaveWrapper::HandleRequestStreamsCommand(pipe::PipeClient *pclient,
                                                 protocol::sequance_id_t id,
                                                 int argc, char *argv[]) {
  UNUSED(id);
  UNUSED(argc);
...
common::ErrnoError
ProcessSlaveWrapper::HandleResponceStreamsCommand(pipe::PipeClient *pclient,
                                                  protocol::sequance_id_t id,
                                                  int argc, char *argv[]) {
  UNUSED(pclient);
  UNUSED(id);
...
void ProcessSlaveWrapper::ReadConfig() { // CONFIG_SLAVE_FILE_PATH
  ...
  if (!utils::ArgsGetValue(slave_config_args, STATS_CREDENTIALS_FIELD,
                           &stats_data)) {
    CRITICAL_LOG() << "Define " STATS_CREDENTIALS_FIELD
                      " variable and make it valid.";
  }

  if (!utils::ArgsGetValue(slave_config_args, LOG_FILE_FIELD, &log_path_)) {
    WARNING_LOG()
        << "Define " LOG_FILE_FIELD
           " variable and make it valid, now used: " DUMMY_LOG_FILE_PATH ".";
    log_path_ = DUMMY_LOG_FILE_PATH;
  }

  stats::StatCredentialsBase *screds =
      stats::StatCredentialsBase::CreateCreadentialsFromString(stats_data);
  CHECK(screds);
  ClearStat();
...
std::string ProcessSlaveWrapper::GetLogPath() const { return log_path_; }
